...
//==============================================================================
class TestingPage::ContentPanel : public juce::Component,
...
public:
    ...
    void resized() override
    {
        ...
        // Grow to fit everything just laid out -- lets the outer viewport's
        // scrollbar reach the last row on any window size. Converges in one
        // extra pass since nothing here depends on height.
        const int neededHeight = progressTextLabel_.getBottom() + 40;
    ...
    std::function<void(const TestingPage::SeedOptions& opts,
...
private:
    ...
    TestingPage::SeedOptions readOptions() const
    {
        TestingPage::SeedOptions o;
    ...
    : content_ (std::make_unique<ContentPanel>())
{
    ...
    content_->onApplyResolution = [this](int w, int h) { if (onApplyResolution) onApplyResolution(w, h); };
    content_->onCreateQueue = [this](const SeedOptions& opts,
                                     std::function<void(float)> onProgress,
                                     std::function<void(bool, juce::String)> onDone)
    {
        if (onCreateQueue) onCreateQueue(opts, std::move(onProgress), std::move(onDone));
    };
}

TestingPage::~TestingPage() = default;
